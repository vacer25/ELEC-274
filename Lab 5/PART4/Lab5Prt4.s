...
# Symbol definitions
	...
	.equ	ESC_CHAR, 0x1B				# ESC character
...
repeat:
	...
	stw		r0, Hex(r0)		# initialize hex result
    movi 	r7, 5			# character counter
...
Get5:
	...
	movui	r4, '0'
	movui	r5, '9'
    bltu	r2, r4, CheckEscapeChar
    bgtu	r2, r5, CheckEscapeChar
...
CheckEscapeChar:
	movi	r5, ESC_CHAR	# was it escape character?
	beq		r2, r5, Display	# if yes, display the number so far
	br		BadChar			# if no, this is a bad character
...
#     subi	r8, r8, 1
# Times10:
#     beq		r8, r0, AddCurrentNum
#     mov		r3, r2			# Store original input number
#     slli	r3, r3, 1		# r3 <- r3 * 2
#     slli	r2, r2, 3		# r2 <- r2 * 8
...
AddCurrentNum:
	...
	# Multiply current number in r6 by 10
	mov		r3, r6			# Store original number
    slli	r3, r3, 1		# r3 <- r3 * 2
    slli	r6, r6, 3		# r6 <- r6 * 8
    add 	r6, r6, r3		# r6 <- r6 + r3
	
	add 	r6, r6, r2		# add the current number
	stw		r6, Num(r0)		# store number for next time
	# Got enough yet?
	subi	r7, r7, 1		# decrement character counter
	bne		r7, r0, Get5	# not done, get another digit

#
# 	# Got 5 decimal digits, convert to hex
# 	mov 	r7, r0			# current digit
# 	movi	r3, 5			# digit to stop at
# 	
# Dec2Hex:
# 	ldw		r6, Num(r0)		# Get number so far
# 	mov		r8, r0			# Division result
# 	mov		r9, r6			# Division remainder
# 	movi	r5, 16			# Number to divide by
# DivLoop:
# 	blt		r9, r5, DoneDiv	# If current sum < 16, done division
# 	sub		r9, r9, r5		# Current number -= number to divide by
# 	addi	r8, r8, 1		# Add 1 to division result
#     br		DivLoop			# Do it again
# DoneDiv:
# 	mov		r6, r8			# New number = division result
# 	stw		r6, Num(r0)		# Store number for next time
# 	
# 	ldw		r11, Hex(r0)	# Get hex number so far
# 	
# 	slli	r10, r7, 2		# Calculate how much to shift by
# 	sll 	r8, r9, r10		# shift over current amount of bits to put hex digit in right place
# 	or		r11, r11, r2	# or the current hex digit to the current hex number being built up
# 
# 	stw		r11, Hex(r0)	# Store hex number for next time
# 	
# 	addi	r7, r7, 1		# decrement digit counter
# 	bne		r7, r3, Dec2Hex # not done, get convert another digit
	
	# Show number on 8 segment display
Display:
	# mov		r2, r11
	mov		r2, r6
	call	Show8Seg
...
#==============================================================================
# Static data.
Prompt:	.asciz	"Type 5 decimal digits: "
...
Hex:	.word	0		# used to hold the calculated hex number
